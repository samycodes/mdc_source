...
class CardController extends Controller
{
        ...
        public function addSingleCard(Request $request){
            ...
            $check_card = Card::where('gcc_id', $request->input('gcc_id'))->get();

            $current_date = Carbon::now();
            
            if($check_card->count() > 0){
                foreach($check_card as $obj){
                  
                    $year = date("Y-m-d", strtotime($obj->occured_on));
                    $future_date = date('Y-m-d', strtotime('+1 year', strtotime($year)) );
                   
                    if($future_date > $current_date){
                        return response()->json(["status" => 0, "message" => "GPR ID is existing ! Please check your unique GPR ID Register Single Card"]);
                    }
                }
            }
            
           if($request->input('document_upload') != '0' && $request->input('document_upload')  != ''){
        ...
        public function addFamilyCard(Request $request){
            ...
            foreach ($data as $val)
            {
                ...
                $check_card = Card::where('gcc_id', $val['gcc_id'])->get();
                ...
                if($check_card->count() > 0){
                    foreach($check_card as $obj){
                        ...
                        if($future_date > $current_date){
                            return response()->json(["status" => 0, "message" => "GPR ID is existing ! Please check your unique GPR ID For Register Family Card"]);
                        }
                    }
                }
                ...
                if($val['document_upload'] != '0' && $val['document_upload']  != ''){
                    $base64_image = 'data:image/jpeg;base64,' . $val['document_upload'];
                    if (preg_match('/^data:image\/(\w+);base64,/', $base64_image)) {
                        $data = substr($base64_image, strpos($base64_image, ',') + 1);
                        $data = base64_decode($data);
                        $filename = time().$data1. '-image.png';
                        Storage::disk('public')->put($filename , $data);
                        $card->document_upload = 'http://128.199.31.19/mdc/storage/app/public/'.$filename;
                    }
                 } else{
        ...
        public function cardHistory(Request $request){
            ...
            if($user != "" && isset($user)){
                $cards = Card::orderBy('id','desc')->select('id','fullname','policy','gcc_id','card_status','created_date','accepted_date','expired_date')
