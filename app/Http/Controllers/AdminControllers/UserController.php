...
class UserController extends Controller
{
    ...
    public function password(Request $request){
        ...
        $validator = \Validator::make($request->all(),[
            ...
            'confirm' => 'required|required_with:new|same:new|min:6',    
        
        ],[
            'current.required' => 'Your Curent Password Field Is Required !',
            'new.required' => 'Your New Password Field Is Required !',
            'confirm.required' => 'Your Confirm Password Field Is Required !',
            'confirm.same' => 'Your New Password & Confirm Password Must Be Same !',
            'confirm.required_with' => 'Your New Password & Confirm Password Must Be Same !',
            'new.min' => 'Password should be at least 6 characters in length !',
            'new.regex' => 'Password should be at least 6 characters in length and should include at least one upper case letter, one number, and one special character!'
        ]);
        ...
        if (!(\Hash::check($request->get('current'), $user->password))) {
            return response()->json(array('error' => 'Your current password does not matches with the Old password you provided. Please try again'));
        }

        if(strcmp($request->get('current'), $request->get('new')) == 0){
            return response()->json(array('error' =>'New Password cannot be same as your current password. Please choose a different password'));
        ...
        return response()->json(array('success' => 'Password changed successfully'));
    ...
    public function listing(){
        $contact = GetinTouch::orderBy('id','desc')->get();
        return view('backend.contactus.index', compact('contact'));
    }
    ...
    public function approveCrd(Request $request)
    {
      ...
      if($card_id=="")
      {
        ...
        if($card_status == 'rejected'){
            $data = array('mtitle' =>'New Notification','mdesc' => 'Your All Card Is Rejected' );
        }else{
            $data = array('mtitle' =>'New Notification','mdesc' => ' Your All Card Is Approved' );
        }

       $userToken = AccessToken::select('device_token')->where('user_id', $id)->where('device_token', '!=', null)->get();     
	...
	$this->pushNotification($data, $device_token);
	 ...
	 // $user = User::where('notification_status','on')->where('id',$id)->first();
                  $userToken = AccessToken::select('device_token')->where('user_id', $user->id)->where('device_token', '!=', null)->get();     
