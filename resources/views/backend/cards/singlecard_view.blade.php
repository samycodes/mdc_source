...
<div class="row single_card_row">
    ...
    left: 472px;">BHD {{$p1}}</span></h3><span style="width: 447px; height: 40px;" class="float-right badge  text-capitalize bg-disable">
                    ...
                    <form class="myForm" id="myForm" action="{{$user->id}}" method="post" enctype="multipart/form-data" autocomplete="nop">
                      ...
                      <input type="hidden" name="card_id" value="">
                      {{csrf_field()}}
                      <button class="btn bg-navy submitdata Rejected" id="reject" type="submit" style="position: relative;top: -41px;">Rejected All</button>
                    </form>
                  </span>
...
<div class="col-md-4">
	...
	@if($value->document_upload != '')<img src="{{ $value->document_upload }}"  alt="img" style="object-fit: cover;height: 51px;width: 79px; margin-left: 13%;" class="demo-image first">@endif
          @endif
           @if($value->documenttypefunction['name'] != 'None' || $value->document_upload != '')  
